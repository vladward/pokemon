...
const USED_SPRITES = [
  'front_default',
  'other_official-artwork_front_default',
] as const;
...
type SpeciesMetaRow = { genus: string | null; flavor_text: string | null; egg_groups: string | null };
...
async function resolveSpeciesMeta(
  ...
  const [row] = await db.$queryRaw<SpeciesMetaRow[]>`
    SELECT
      (SELECT genus FROM pokemon_species_genus
       WHERE species_id = ${speciesId} AND language = ${locale} LIMIT 1) AS genus,
      (SELECT flavor_text FROM pokemon_species_flavor_text
       WHERE species_id = ${speciesId} AND language = ${locale}
      ...
      (SELECT GROUP_CONCAT(COALESCE(egn.name, eg.name) ORDER BY eg.id SEPARATOR '|||')
       ...
       JOIN egg_group eg ON pseg.egg_group_id = eg.id
...
async function fetchPokemonById(
  ...
  const [speciesMeta, evolution, locations, forms] = await Promise.all([
    speciesId ? resolveSpeciesMeta(speciesId, locale) : { genus: null, flavorText: null, eggGroups: [] as string[] },
