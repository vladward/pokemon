...
async function fetchPokemonNeighbors(
  ...
  const [row] = await db.$queryRaw<{ prev_id: number | null; next_id: number | null }[]>`
        SELECT (SELECT id FROM pokemon WHERE id < ${id} ORDER BY id DESC LIMIT 1) AS prev_id,
...
export const getPokemonNeighbors = unstable_cache(fetchPokemonNeighbors, ['pokemon-neighbors'], {
  revalidate: false,
});
