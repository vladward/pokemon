...
const getCalledParams = () => new URLSearchParams((mockPush.mock.calls[0][0] as string).slice(1));
...
describe('Pagination', () => {
  ...
  describe('page range', () => {
    it('shows all pages when totalPages <= 7', () => {
      ...
      [1, 2, 3, 4, 5, 6, 7].forEach((n) => expect(screen.getByText(String(n))).toBeInTheDocument());
  ...
  describe('navigation', () => {
    ...
    it('preserves search in URL when navigating between pages', () => {
      mockUseSearchParams.mockReturnValue(makeSearchParams({ search: 'pikachu', page: '1' }));
      render(
        <Pagination
          page={1}
          totalPages={3}
