...
const makeStep = (fromSpeciesId: number, toSpeciesId: number, minLevel: number | null = null): TEvolutionStep => ({
  fromSpeciesId,
  toSpeciesId,
  trigger: 'level-up',
  minLevel,
...
describe('PokemonEvolutionPanel', () => {
  it('renders nothing when evolution list is empty', () => {
    const { container } = render(<PokemonEvolutionPanel evolutions={[]} evolutionSteps={[]} />);
    expect(container).toBeEmptyDOMElement();
  });

  it('renders a single-stage pokemon with no arrows', () => {
    render(<PokemonEvolutionPanel evolutions={[makeNode(132, 'ditto', true)]} evolutionSteps={[]} />);
  ...
  it('renders a linear three-stage chain in order', () => {
    ...
    render(<PokemonEvolutionPanel evolutions={nodes} evolutionSteps={steps} />);
  ...
  it('shows the scroll indicator for a branching chain', () => {
    ...
    const nodes = [makeNode(133, 'eevee', true), makeNode(134, 'vaporeon'), makeNode(135, 'jolteon')];
  ...
  it('renders the Evolution Chain section label', () => {
    ...
    render(<PokemonEvolutionPanel evolutions={nodes} evolutionSteps={[]} />);
